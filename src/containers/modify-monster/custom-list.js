...
const CustomList = ({ title, description, fieldKey, values, onValueChange }) => {
    ...
    const handleChange = useCallback((id, name, note) => {
        ...
        <div>
            ...
            <ul className="list-table">
                ...
                {values.length > 0 ? values.map((value) => (
                    <li key={value.id}>
                        <label>
                            <input aria-label="name" type="text" name={`${fieldKey}`} value={value.name} onChange={(e) => handleChange(value.id, e.target.value, value.note)} />
                        </label>
                        <label>
                            <input aria-label="note" type="text" name={`${fieldKey}`} value={value.note} onChange={(e) => handleChange(value.id, value.name, e.target.value)} />
                        </label>
